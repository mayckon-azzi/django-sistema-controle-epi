...
def test_movimenta_por_entrega_create_and_update_and_delete():
    ...
    movimenta_por_entrega(e, antiga=None)  # -3
    epi.refresh_from_db()
    ...
    movimenta_por_entrega(e2, antiga=antiga)  # delta: 0 - (-3) = +3
    epi.refresh_from_db()
    assert epi.estoque == 10

    # exclusão de um registro perdido (efeito -q) deve repor (+q)
    e3 = Entrega(epi=epi, quantidade=2, status=Entrega.Status.PERDIDO)
    movimenta_por_entrega(e3, antiga=None)  # -2
    epi.refresh_from_db()
    assert epi.estoque == 8
    movimenta_por_exclusao(e3)              # +2
    epi.refresh_from_db()
    assert epi.estoque == 10
...
def test_movimenta_por_entrega_insuficiente():
    ...
    epi = EPI.objects.create(codigo="C1", nome="Capacete", categoria=cat, estoque=2, estoque_minimo=0)
