...
def test_criar_entrega_updates_stock_and_shows_message(client):
    ...
    resp = client.post(url, data={
        ...
        "data_prevista_devolucao": (timezone.now() + timedelta(days=3)).strftime("%Y-%m-%dT%H:%M"),
        "observacao": "teste",
    }, follow=True)
...
def test_atualizar_entrega_and_delete_adjusts_stock_and_messages(client):
    ...
    resp = client.post(url_edit, data={
        "colaborador": col.pk,
        "epi": epi.pk,
        "quantidade": 1,
...
def test_solicitacoes_criar_aprovar_reprovar_atender_e_acoes_rapidas(client):
    ...
    resp = client.post(url_new, data={"epi": epi.pk, "quantidade": 2, "observacao": ""}, follow=True)
    assert resp.status_code == 200
