...
def test_context_aggregations_and_filters(client):
    ...
    # registros (dois fora do estoque, um devolvido)
...
def test_export_csv_filters_and_format(client):
    ...
    # dois registros, vamos filtrar por status = DEVOLVIDO
    Entrega.objects.create(
    ...
    # apenas 1 linha de dados (filtrada)
    assert len(rows) == 2
    data_row = rows[1]
    # checagens simples de colunas chave
    assert data_row[3] == "Bruno"
    assert data_row[4].startswith("Cap (C1)")
    assert data_row[5] == "4"
    assert data_row[6].lower() == "devolvido"
    # observação sem quebras de linha
